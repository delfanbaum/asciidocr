    ...
    pub fn inlines(&self) -> Vec<&Inline> {
        ...
        match self {
            ...
            Block::ListItem(block) => {
                inlines.extend(block.principal.iter());
                for child in block.blocks.iter() {
                    inlines.extend(child.inlines())
                }
            }
            ...
            Block::LeafBlock(block) => inlines.extend(block.inlines.iter()),
            Block::TableCell(block) => inlines.extend(block.inlines.iter()),
